...
mydata <- read.csv("./Data files/ecg_feature_exports/B_cells.csv", header=T)
all_data <- mydata
...
all_mean_s <- as.matrix(scale(all_data[, c(1:9)], center=as.matrix(control_mean[, c(2:10)]), scale = as.matrix(control_sd[, c(2:10)])))
